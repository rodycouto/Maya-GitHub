...
exports.run = async (client, message, args) => {
	...
	if (!args[0]) { return message.inlineReply('Convide alguém para usa familia! Pode até 3 pessoas. `' + prefix + 'family3 @user`') }
	...
	if (db.get(`family3_${message.author.id}`)) { return message.inlineReply(`Nesta posição, <@${db.get(`family3_${message.author.id}`)}> é seu familiar.`) }
	if (db.get(`family3_${member.id}`)) { return message.inlineReply(member.username + ' já tem um familiar nesta posição.') }
	...
	if (family === null && family2 === null) {
		...
		return message.inlineReply(`<a:attention:836101248183959562> Tem algo errado. Parece que algúm dos dois tem algo no Family 3.`)
	}
}